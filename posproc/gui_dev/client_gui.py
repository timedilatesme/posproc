...
def handle_post_processing_button(event):
    if event == START_POST_PROCESSING_EVENT:
        code_client_backend = """from posproc import* 
# access data
...
bob.stopClient()"""
        # with open("client_backend.py", 'w') as fh:
        #     fh.writelines(code_client_backend)
        
        client_backend = subprocess.Popen('client_backend.exe ' + parameters_data_path,
